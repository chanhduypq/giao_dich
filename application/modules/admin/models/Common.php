...
class Admin_Model_Common {
     ...
     */
    public static function build_array_for_level($items, &$level_1_items, &$level_2_items, &$level_3_items) {
        $level_1_items = array();
        ...
        foreach ($items as $item) {
            if (is_numeric($item['id2'])) {
                $level_3_items[$item['id2']] = array('name_show'=>$item['name_show2'], 'is_show_at_home_page' => $item['is_show_at_home_page2'],'photo'=>$item['photo2'], 'name' => $item['name2'], 'parent_id' => $item['id1']);
